...
| **L3 테넌트 설정** | 타임존·위젯(문구·테마·커스텀·디자인 파일)·임베드·로그인 방식·개인정보 URL/동의 버전·알림 채널·연동 자격증명·핸드오프·KB·AI 설정 | 테넌트 master/director | 즉시(위젯은 다음 로드) | **설정 스냅샷**(12필드+커스텀 위젯) · 커스텀 위젯 패키지 · KB 일괄 내보내기/등록 · 자격증명은 재입력 |
...
| 고객(위젯) 언어 판정 | L4→L3 | `session.service.ts resolveLanguage`: 브라우저 locale → 없으면 **테넌트 타임존**으로 유도 → 없으면 `en` | 타임존이 유일한 테넌트 레버(G1/G2) |
...
| 업무시간 타임존(핸드오프) | L4 | `HandoffSection.tsx` 목록: 미국 4 + Seoul + UTC | **호찌민 없음**(G3) |
...
| AI 국외이전 고지 | L4 | 위젯 i18n `chat.aiDisclosure` — "**in the United States**" 고정 | 추론 위치가 바뀌면 오문(G7) |
...
| PII 로그 스크럽 전화 패턴 | L4 | `pii-scrub.util.ts`: 국제·미국·한국·generic | **베트남 이동전화(`0[35789]…`, `+84`) 없음**(G6) |
| 메신저 동의 모드 | L3 | 채널별 `notice`/`auto` | 첫 접촉 시 고지 발송 여부 |
| GA4 동의 모드 | L1 | `VITE_GA4_MEASUREMENT_ID`(빌드) | `analytics_storage` 기본 denied |
| 사고 통지 기한 | 문서 | `INCIDENT-RESPONSE.md` §4: GDPR 72h·CCPA·PIPA | **PDPD 행 없음**(G10) |
...
| 시드 테넌트·KB·시나리오 | L4 | `seed.runner.ts`: 테넌트 `ivyusa`, 미국 화장품 KB("30일 반품, Sephora/Ulta"), 시나리오 스크립트 동일 정책(G11) |
...
| 3 | AI 처리 위치 고지·DPA | 문서/L4 | 고지문에 미국 처리 명시(G7), Anthropic DPA | 동일 |
| 4 | 보존 기간(대화·AI 사용량) | L1 | 처리방침과 일치 | 동일 |
| 5 | 기본 언어·지원 언어 | L4/L3 | vi 기본(G2 선행), vi/en 병기 | en |
| 6 | 타임존 | L3 | `Asia/Ho_Chi_Minh`(핸드오프 목록 G3) | `America/New_York`·`Los_Angeles` |
...
| 14 | 초기 KB 출처 | L3 | 호텔 운영 KB(vi/en), 시드 KB 삭제 | 시드 KB(미국 화장품) 유지·보강 |
...
3. **대상 환경**(테넌트 master)
   - 설정 스냅샷은 **같은 테넌트에 저장된 것만 복원**할 수 있습니다 → 다운로드 JSON은 대조용으로 열어 두고 항목을 **수동 입력**(12항목 + 타임존). 커스텀 위젯은 **[패키지 가져오기]**로 복원 후 [사용함]
...
| # | 갭 | 위치 | 권장 처리 | 우선 | 국가 |
|---|---|---|---|---|---|
| G1/G2 | 기본 언어 `en` 고정, `tenants.default_language` 없음 → 브라우저가 en-US면 타임존이 호찌민이어도 영어 | `session.service.ts:557-564`, `apps/web/src/i18n/i18n.ts` | `tenants.default_language` 컬럼 + 판정 순서 `명시 locale > 테넌트 기본 > 타임존 > en` + 콘솔 초기 언어 | **P0** | VN |
| G3 | 핸드오프 타임존 목록에 `Asia/Ho_Chi_Minh` 없음 | `HandoffSection.tsx:15-22` | `LANGUAGE_TIMEZONES` 재사용 또는 IANA 입력 허용 | **P0** | VN |
| G6 | 베트남 전화 PII 스크럽 패턴 없음 | `pii-scrub.util.ts:62-74` | `PHONE_VN_RE`(`(\+84\|0)(3\|5\|7\|8\|9)\d{8}`) 추가 + 테스트 | **P0** | VN |
| G7 | AI 국외이전 고지 "in the United States" 고정 | 위젯 i18n `chat.aiDisclosure` ×6 | 처리 지역을 env(`AI_PROCESSING_REGION`)로 받아 문구 치환, 또는 테넌트 개인정보 안내 필드 | **P0** | VN(고지 의무), 전체 |
| G11 | 시드 KB·시나리오가 미국 화장품 정책 | `seed.runner.ts:193-203`, `scenario-scripts.ts:52-56` | `SEED_KB_PROFILE`(none/us-cosmetics) + 시나리오 정책 문구를 테넌트 설정으로 | **P0**(VN은 삭제로 우회 가능) | VN |
| G10 | 사고 통지 매트릭스에 PDPD 없음 | `INCIDENT-RESPONSE.md` §4 | 행 추가(72h, 개인정보보호위원회 A05) | P0(문서) | VN |
