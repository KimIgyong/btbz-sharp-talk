# AMA 포털 → SharpTalk iframe SSO 연동 가이드

ama.amoeba.site(ambManagement)에서 SharpTalk 콘솔을 iframe으로 임베드하고 원클릭 SSO로
진입시키는 설정. SharpTalk 측 구현: PLN-260813-AMA-Iframe-SSO (S1~S3).
**ama 레포 작업자용 명세** — SharpTalk 쪽은 배포 완료 상태를 전제.
...
ama 웹(부모) ──iframe──▶ shoptalk.amoeba.site/{tenant_slug}?ama_token={SSO JWT}
                          ...
                          │ 3. SharpTalk API → ama /oauth/token (grant_type=ama_session,
                          │    client_id/secret) — ama가 서명·만료·회수·사용자상태 검증
                          │ 4. ama_token의 email 클레임 ↔ 해당 테넌트의 활성 계정 매핑
                          └▶ 5. SharpTalk 자체 JWT 발급 → 일반 콘솔 세션과 동일
```
...
| 앱 이름 | SharpTalk |
| 스코프 | `profile` (최소) |
| 발급물 | client_id / client_secret → SharpTalk 운영자에게 전달(안전 채널) |

SharpTalk 스테이징 env에 반영: `AMA_SSO_TOKEN_URL=https://ama.amoeba.site/api/v1/oauth/token`,
...
| 요건 | 이유 |
|---|---|
| **`email` 클레임 포함** | SharpTalk 계정 매핑 축. ama `/oauth/userinfo`는 email을 반환하지 않으므로 토큰 클레임이 유일한 전달로 — email 없으면 E5033 거절 |
...
| MFA (D4) | SSO 진입은 SharpTalk MFA 면제(ama 인증 신뢰). 최초 비밀번호 변경 잠금은 유지 |
...
## 4. 검증 절차 (등록 후)
1. ama에 로그인한 브라우저에서 SharpTalk 메뉴 클릭 → iframe 콘솔이 로그인 화면 없이 열림.
2. SharpTalk 감사 로그에 `auth.sso_ama` 1건.
3. ama 계정 email이 SharpTalk에 없는 사용자로 재시도 → "SSO 로그인 실패" 배너 + 일반 로그인 폼.
