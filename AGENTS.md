# AGENTS.md — btbz-SharpTalk

> ⚠️ GENERATED MIRROR of `CLAUDE.md` for non-Claude coding agents (Codex 등).
> Do not edit directly — edit `CLAUDE.md`, then re-mirror (title, the two
> [mirror]-marked lines, and the `.agents/skills/` copies).
...
the project skill `.agents/skills/sharptalk-dev/SKILL.md`. <!-- [mirror] -->
...
- **RBAC**: rank (master/director/manager/staff) × label (consult/accounting/operations) via `@sharptalk/common/permission-matrix`; system admin super/admin. ACL owner-visibility (POL-019) above it.
...
- **i18n**: NO hardcoded UI text — use `t()` from `useTranslation()`; register namespaces in each app's `i18n.ts` (console: `import.meta.glob`); `fallbackLng: 'en'`; locales en/es/ko/vi/ja/zh. The language set lives ONCE in `packages/types/src/common/language.ts` (codes, endonyms, session values, timezone defaults, review state) — never re-list languages in an app. ⚠️ Browser bundles deep-import that source file: a value import from `@sharptalk/types` fails the build (CJS `export *`). Backend conversational strings localized by `session.language`; AI/RAG answers honor it. Run `npm run i18n:check` after touching locales — a missing key is a silent English fallback otherwise.
...
values change (ports/domains/creds), update `CLAUDE.md`+`AGENTS.md`/`SPEC.md`/`CONFIG.md` immediately; <!-- [mirror] -->
...
```bash
scripts/session-worktree.sh new <name>   # ~/orca/worktrees/btbz-sharptalk/<name>, 브랜치 session/<name>
